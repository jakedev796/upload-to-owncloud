...
async function loadFolders() {
    ...
    const { serverURL, username, password } = await browser.storage.sync.get(['serverURL', 'username', 'password']);
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const items = await browser.storage.sync.get(['serverURL', 'username', 'password', 'uploadFolder', 'copyUrlToClipboard']);
...
document.getElementById('save').addEventListener('click', async function () {
    ...
    if (connectionSuccessful) {
        await browser.storage.sync.set({ serverURL, username, password, copyUrlToClipboard });
...
document.getElementById('saveFolder').addEventListener('click', async function () {
    await browser.storage.sync.set({ uploadFolder: currentPath });
